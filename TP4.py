...
# Input root node
root_node = int(input("Enter the root node (1-9): ")) - 1  # Convert to 0-based index

# Run Prim's algorithm
prim_mst_edges, prim_total_weight = prim(graph, root_node)
print("\nPrim's Algorithm - MST edges and weights:")
for edge in prim_mst_edges:
    print(f"Edge: {edge[0]}-{edge[1]} with weight {edge[2]}")
print(f"Total weight of MST (Prim's): {prim_total_weight}")

# Run Kruskal's algorithm
kruskal_mst_edges, kruskal_total_weight = kruskal(graph)
print("\nKruskal's Algorithm - MST edges and weights:")
for edge in kruskal_mst_edges:
    print(f"Edge: {edge[0]}-{edge[1]} with weight {edge[2]}")
print(f"Total weight of MST (Kruskal's): {kruskal_total_weight}")
